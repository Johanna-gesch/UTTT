...
public class MortalKomBOTBeta implements IBot{
    final int moveTimeMs = 1000;
    private static final String BOTNAME = "Mortal KomBOT Beta";
    ...
    private GameSimulator createSimulator(IGameState state) {
        ...
        // 1) Lav en dyb kopi af board (9x9)
        ...
        // 2) Lav en dyb kopi af macroboard (3x3)
    ...
    private IMove calculateWinningMove(IGameState state, int maxTimeMs) {
        ...
        while (true) {
            ...
            Double C = 1.4;
            IMove firstMove = moves.get(rand.nextInt(moves.size()));
            ...
            while (simulator.getGameOver() == GameOverState.Active) {
                ...
                moves = simulator.getAvailableMoves();
        ...
        // Find træk med bedst winrate
        double bestScore = -1;
        IMove bestMove = null;

        for (IMove move : plays.keySet()) {
            int p = plays.get(move);
            int w = wins.getOrDefault(move, 0);
            double score = (double) w / p;
    ...
    private List<IMove> getSafeWinningMoves(IGameState state){
     ...
     * @return liste
     */
    private List<IMove> getOpponentImmediateThreats(IGameState state) {

        int opponent = 1 - (state.getMoveNumber() % 2);
        return getWinningMovesForPlayer(state, String.valueOf(opponent));
     ...
     * @return liste.
     */
    private List<IMove> getWinningMovesForPlayer(IGameState state, String player) {
    ...
    private IMove getImmediateMacroWinMove(IGameState state) {
        int myPlayer = state.getMoveNumber() % 2;
